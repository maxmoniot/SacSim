...
class BackpackSimulatorApp {
    ...
    resetPosition() {
        ...
        if (this.viewer.stlMesh) {
            ...
            // Mettre à jour la flèche du point d'accrochage
            if (this.viewer.updateHangingPointPosition) {
                this.viewer.updateHangingPointPosition();
            }
        }
    }
    ...
    resetRotation() {
        ...
        if (this.viewer.stlMesh) {
            ...
            // Supprimer le point d'accrochage car la rotation a changé
    ...
    updatePosition(axis, value) {
        ...
        if (this.checkTableCollision()) {
            ...
            // Mettre à jour la position de la flèche du point d'accrochage
            if (this.viewer.updateHangingPointPosition) {
                this.viewer.updateHangingPointPosition();
            }
        }
     ...
     */
    checkTableCollision() {
        if (this.skipCollisionCheck) return false;
    ...
    updateRotation(axis, value) {
        ...
        if (this.checkTableCollision()) {
            ...
            // Supprimer le point d'accrochage car la rotation a changé
            // (la flèche ne doit pas pointer vers le haut)
            if (this.viewer.hangingPoint) {
                this.clearHangingPoint();
                this.ui.showToast('Info', 'Point d\'accrochage supprimé suite à la rotation', 'info');
            }
        }
    }
     ...
     */
    checkSupportOnTable() {
        if (this.skipCollisionCheck) return true;
    ...
    async runSimulation() {
        ...
        // === DEBUG CAPTURE ===
        const simDebug = [];
        simDebug.push('══════ APP ÉLÈVE ══════');
        simDebug.push('');
        simDebug.push('── Paramètres géométrie ──');
        simDebug.push('geometry.center() = OUI (appliqué au chargement)');
        simDebug.push('vertices count = ' + this.currentSTLFile.geometry.attributes.position.count);
        ...
        simDebug.push('materialCoefficient = ' + this.materialCoefficient);
        ...
        if (this.simulation2D) {
            try {
                ...
                simDebug.push('');
                simDebug.push('── extractProfile ──');
                
                if (profile && profile.points && profile.points.length > 0) {
                    simDebug.push('profile points = ' + profile.points.length);
                    simDebug.push('Z width = ' + (profile.zWidth || 0).toFixed(2) + ' cm');
                    simDebug.push('bounds X = [' + profile.bounds.xMin.toFixed(2) + ', ' + profile.bounds.xMax.toFixed(2) + ']');
                    simDebug.push('bounds Y = [' + profile.bounds.yMin.toFixed(2) + ', ' + profile.bounds.yMax.toFixed(2) + ']');
                    // Échantillons d'épaisseurs
                    const samples = [];
                    for (let pi = 0; pi < profile.points.length; pi += Math.max(1, Math.floor(profile.points.length / 6))) {
                        const s = profile.points[pi];
                        samples.push('Y=' + s.y.toFixed(2) + ' h=' + s.thickness.toFixed(2));
                    }
                    simDebug.push('épaisseurs: ' + samples.join(' | '));

                    // Analyser la structure
                    const structure = this.simulation2D.analyzeStructure(this.viewer.hangingPoint);

                    simDebug.push('');
                    simDebug.push('── analyzeStructure ──');
                    
                    if (structure && structure.freeSections && structure.freeSections.length > 0) {
                        simDebug.push('sections ancrées = ' + structure.anchorSections.length);
                        simDebug.push('sections libres = ' + structure.freeSections.length);
                        simDebug.push('pivot = (' + structure.pivot.x.toFixed(3) + ', ' + structure.pivot.y.toFixed(3) + ')');
                        simDebug.push('leverArm = ' + (structure.leverArm * 100).toFixed(2) + ' cm = ' + structure.leverArm.toFixed(5) + ' m');
                        simDebug.push('zWidth = ' + (structure.zWidth * 100).toFixed(2) + ' cm = ' + structure.zWidth.toFixed(5) + ' m');
                        const minThick = Math.min(...structure.freeSections.map(s => s.thickness));
                        simDebug.push('épaisseur libre min = ' + minThick.toFixed(3) + ' cm');
                        ...
                        simDebug.push('');
                        simDebug.push('── calculateMaxLoad ──');
                        simDebug.push('maxWeight brut = ' + (maxWeight2D / this.materialCoefficient).toFixed(2) + ' kg');
                        simDebug.push('× coefficient = ' + this.materialCoefficient);
                        simDebug.push('maxWeight final = ' + maxWeight2D.toFixed(2) + ' kg');
                    } else {
                        simDebug.push('⚠️ Pas de sections libres détectées');
                        console.log('⚠️ Pas de sections libres détectées');
                    }
                } else {
                    simDebug.push('⚠️ Profil 2D vide');
                    console.log('⚠️ Profil 2D vide');
                }
            } catch (error) {
                simDebug.push('❌ Erreur: ' + error.message);
                console.error('❌ Erreur simulation 2D:', error);
            }
        }

        // Store debug for display
        this._lastSimDebug = simDebug;
    ...
    displayAdvancedSimulationResults(result) {
        ...
        // === DEBUG PANEL ===
        let debugPanel = document.getElementById('simDebugPanel');
        if (!debugPanel) {
            debugPanel = document.createElement('div');
            debugPanel.id = 'simDebugPanel';
            debugPanel.style.cssText = 'margin-top:10px;';
            resultsDiv.appendChild(debugPanel);
        }
        const debugLines = this._lastSimDebug || ['(pas de debug)'];
        debugPanel.innerHTML = `
            <button id="simDebugToggle" style="font-size:11px;padding:3px 10px;border-radius:8px;background:rgba(80,120,255,.15);color:#6a8fff;border:1px solid rgba(80,120,255,.25);cursor:pointer;margin-top:8px;">🔧 Debug Simulation</button>
            <pre id="simDebugContent" style="display:none;margin-top:6px;padding:10px;background:#0a0e1a;border-radius:8px;font-family:monospace;font-size:10px;line-height:1.6;color:#8892b0;max-height:300px;overflow-y:auto;white-space:pre-wrap;border:1px solid rgba(45,50,80,.5);">${debugLines.join('\n')}</pre>
        `;
        document.getElementById('simDebugToggle').onclick = function() {
            const c = document.getElementById('simDebugContent');
