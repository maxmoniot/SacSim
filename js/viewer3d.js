...
class Viewer3D {
    ...
    setHangingPoint(point) {
        ...
        // Stocker le point en coordonnées LOCALES du mesh STL pour le suivi
        if (this.stlMesh) {
            const localPoint = point.clone();
            // Inverser les transformations du mesh pour obtenir les coordonnées locales
            localPoint.sub(this.stlMesh.position);
            const inverseRotation = new THREE.Euler(
                -this.stlMesh.rotation.x,
                -this.stlMesh.rotation.y,
                -this.stlMesh.rotation.z,
                'ZYX' // Ordre inverse
            );
            localPoint.applyEuler(inverseRotation);
            // Inverser l'échelle (0.1)
            localPoint.multiplyScalar(10);
            this.hangingPointLocal = localPoint;
        }
        ...
        // Supprimer l'ancien overlay 2D s'il existe
        if (this.targetOverlay) {
            this.targetOverlay.remove();
            this.targetOverlay = null;
        }
        
        // Créer une flèche 3D pointant vers le bas
        this.hangingPointMarker = this.create3DArrow(point);
        this.scene.add(this.hangingPointMarker);
    }
    
    /**
     * Met à jour la position de la flèche et du hangingPoint quand le support est déplacé/tourné
     * Doit être appelé après chaque modification de position/rotation du stlMesh
     */
    updateHangingPointPosition() {
        if (!this.hangingPointLocal || !this.stlMesh || !this.hangingPointMarker) return;
        
        // Recalculer la position monde à partir des coordonnées locales
        const worldPoint = this.hangingPointLocal.clone();
        worldPoint.multiplyScalar(0.1); // Appliquer l'échelle
        worldPoint.applyEuler(this.stlMesh.rotation);
        worldPoint.add(this.stlMesh.position);
        
        // Mettre à jour la position de la flèche
        this.hangingPointMarker.position.copy(worldPoint);
        
        // Mettre à jour hangingPoint pour la simulation
        this.hangingPoint = worldPoint.clone();
    }
    
    /**
     * Crée une flèche 3D avec la pointe au point spécifié, pointant vers le bas
     */
    create3DArrow(tipPoint) {
        const arrowGroup = new THREE.Group();
        arrowGroup.userData.isHangingPointMarker = true;
        
        // Dimensions de la flèche
        const coneHeight = 0.625;     // Hauteur de la pointe conique
        const coneRadius = 0.25;      // Rayon de la base du cône
        const shaftLength = 1.25;     // Longueur de la tige
        const shaftRadius = 0.1;      // Rayon de la tige
        
        // Matériau rouge vif pour la visibilité
        const arrowMaterial = new THREE.MeshStandardMaterial({
            color: 0xff0000,
            roughness: 0.3,
            metalness: 0.6,
            emissive: 0x330000  // Légère émission pour visibilité
        });
        
        // Cône (pointe de la flèche) - pointant vers le bas
        const coneGeometry = new THREE.ConeGeometry(coneRadius, coneHeight, 16);
        const cone = new THREE.Mesh(coneGeometry, arrowMaterial);
        // Rotation pour que la pointe pointe vers le bas (par défaut le cône pointe vers Y+)
        cone.rotation.x = Math.PI;
        // Position: la pointe du cône doit être à Y=0 (au point cliqué)
        ...
        // Position: juste au-dessus du cône
        shaft.position.y = coneHeight + shaftLength / 2;
        arrowGroup.add(shaft);
        
        // Positionner l'ensemble au point cliqué
        // La pointe de la flèche sera exactement au point
        arrowGroup.position.copy(tipPoint);
    ...
    updateTargetPosition() {
        // Plus besoin de mettre à jour la position 2D car on utilise un objet 3D
    ...
    clearHangingPoint() {
        ...
        this.hangingPointLocal = null;
        
        if (this.hangingPointMarker) {
            // Supprimer proprement tous les composants du marqueur 3D
            this.hangingPointMarker.traverse((child) => {
                if (child.isMesh) {
